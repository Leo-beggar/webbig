...
(function(html) {
    ...
    * ----------------------------------------------------- */
    const ssPhotoswipe = function() {
        ...
        const activityLinks = document.querySelectorAll('.folio-item__thumb-link');
        ...
        folioItems.forEach(function(folioItem, i) {
            ...
            let title = folioItem.querySelector('.folio-item__title');
            let caption = folioItem.querySelector('.folio-item__caption');
            ...
            thumbLink.addEventListener('click', function(e) {
                ...
                const items = [];
                let imageNames = ['', '2', '3'];
                imageNames.forEach(function(suffix, index) {
                    let item = {
                        src  : href.replace('.jpg', suffix + '.jpg'),
                        w    : width,
                        h    : height
                    }
        ...
        activityLinks.forEach(function(link) {
            if (link.getAttribute('href') && link.getAttribute('href').includes('model.jpg') || 
                link.getAttribute('href') && link.getAttribute('href').includes('qinshi.jpg') || 
                link.getAttribute('href') && link.getAttribute('href').includes('tuanwei.jpg')) {
                
                link.addEventListener('click', function(e) {
                    e.preventDefault();
                    
                    let href = this.getAttribute('href');
                    let size = this.dataset.size.split('x');
                    let width = size[0];
                    let height = size[1];
